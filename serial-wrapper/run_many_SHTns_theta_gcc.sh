...
run_it() {
   ...
   nthread=$5
   out=$6
   ./xmain.Linux.intel.SHTns.theta.gcc.exe --run-timing 1 --nr ${nr} --lmax ${lmax} --nfields ${nf} --nloops ${nloops} --output "${out}" --n-threads ${nthread} --walltime 600.0
}

output=timing-results/gcc-openmp-enabled-theta-contiguous-Run-with-intel-onthefly
