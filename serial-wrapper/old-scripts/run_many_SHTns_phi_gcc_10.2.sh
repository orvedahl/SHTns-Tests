...
run_it() {
   ...
   nthread=$5
   out=$6
   ./xmain.Linux.gfortran.SHTns.phi.gcc.exe --run-timing 1 --nr ${nr} --lmax ${lmax} --nfields ${nf} --nloops ${nloops} --output "${out}" --n-threads ${nthread} --walltime 600.0
}

output=timing-results/gcc-10.2-openmp-enabled-phi-contiguous-Run-with-gcc-10.2-onthefly
