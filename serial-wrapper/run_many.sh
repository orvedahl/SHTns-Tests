...
run_it() {
   ...
   nthread=$5
   out=$6
   ./xmain.Linux.intel.exe --run-timing 1 --nr ${nr} --lmax ${lmax} --nfields ${nf} --nloops ${nloops} --output "${out}" --n-threads ${nthread} --walltime 600.0
...
for l in "${lvals[@]}"; do
   run_it ${l} 16 10 30 1 timing_fixed_Nrhs_320.out
done

Nrs=( 2 4 8 16 32 64 128 256)
for Nr in "${Nrs[@]}"; do
   run_it 63 ${Nr} 10 30 1 timing_fixed_lmax_63.out
done

Nfs=( 2 4 8 16 32 64 128 256)
for Nf in "${Nfs[@]}"; do
   run_it 63 16 ${Nf} 30 1 timing_fixed_lmax63_Nr32.out
done
